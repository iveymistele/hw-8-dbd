...
StewartCastleVillage  <- read.csv("~/Desktop/Sp25/DBD/HW8/SCV_Inventory.csv", stringsAsFactors = F)
...
SCVTotalArts <-  StewartCastleVillage %>% 
  summarise(TotalArtifacts = sum(Total.Count))
...
SCVUnitType <-  StewartCastleVillage %>% 
  group_by(Unit.Type) %>% 
  summarise(Artifact.Count = sum(Total.Count))
...
SCVArtFreq <-  StewartCastleVillage %>% 
  group_by(Artifact.Category) %>% 
  summarise(Total.Count = sum(Total.Count)) %>%
  mutate(Percent = Total.Count/sum(Total.Count))


theme_set(theme_bw(base_size = 20))       
ggplot(SCVArtFreq, aes(x=Artifact.Category, y=Percent,
                       fill=Artifact.Category)) +
